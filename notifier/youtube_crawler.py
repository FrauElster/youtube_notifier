...
class YoutubeCrawler:
    ...
    def search(self, *search_terms: str, last_check_datetime: datetime) -> List[str]:
        videos: List[str] = []
        ...
        for search_term in search_terms:
            ...
            for search_result in search_response.get('items', []):
                ...
                if search_result_date < last_check_datetime:
                    break
                title: str = search_result['snippet']['title']
                url: str = f'https://www.youtube.com/watch?v={search_result["id"]["videoId"]}'

                videos.append(f'{search_term}: {title} - {url}')
    ...
    def _load_config() -> Dict[str, str]:
        ...
        config: Dict[str, str] = load_file(STRINGS.CONFIG_FILE)

        for obligated_key in obligated_keys:
            if obligated_key not in config.keys() or not config[obligated_key]:
                raise ValueError(f'{obligated_key} not specified in {STRINGS.CONFIG_FILE}')
